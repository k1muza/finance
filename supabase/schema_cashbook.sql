-- Reference copy — see supabase/migrations/20260415040000_cashbook_engine.sql for history
...
DO $$ BEGIN
  CREATE TYPE public.transaction_status AS ENUM (
    'draft', 'submitted', 'approved', 'posted', 'reversed', 'voided'
  );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- District-scoped transaction sequences
...
CREATE INDEX IF NOT EXISTS idx_cashbook_txn_account          ON public.cashbook_transactions(account_id);
CREATE INDEX IF NOT EXISTS idx_cashbook_txn_district_status  ON public.cashbook_transactions(district_id, status);
...
CREATE INDEX IF NOT EXISTS idx_cashbook_lines_transaction ON public.cashbook_transaction_lines(transaction_id);
CREATE INDEX IF NOT EXISTS idx_cashbook_lines_account     ON public.cashbook_transaction_lines(account_id);
...
CREATE POLICY "district_read_own_cashbook_lines" ON public.cashbook_transaction_lines
  FOR SELECT USING (EXISTS (
    ...
    WHERE t.id = transaction_id AND t.district_id = public.current_district_id()
  ));
CREATE POLICY "district_write_draft_cashbook_lines" ON public.cashbook_transaction_lines
  FOR INSERT WITH CHECK (EXISTS (
    SELECT 1 FROM public.cashbook_transactions t
    WHERE t.id = transaction_id AND t.district_id = public.current_district_id() AND t.status = 'draft'
  ));
...
CREATE INDEX IF NOT EXISTS idx_cashbook_audit_transaction ON public.cashbook_audit_log(transaction_id);
CREATE INDEX IF NOT EXISTS idx_cashbook_audit_created     ON public.cashbook_audit_log(created_at DESC);
