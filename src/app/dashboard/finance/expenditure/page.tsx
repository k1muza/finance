...
import { Currency, Expense, PaymentMethod, CURRENCY_SYMBOLS, PAYMENT_METHOD_LABELS } from '@/types'
...
export default function ExpenditurePage() {
  ...
  const { data: expenses, loading, totalsByCurrency, add, update, remove, refresh } = useExpenses({
  ...
  const handleAdd = async () => {
    ...
    if (!districtId) {
      toast.error('Select a district before saving expenditure.')
      return
    }
  ...
  const startEdit = (expense: Expense) => {
    ...
    setDraft({
      ...
      fund_id: expense.fund_id ?? defaultFundId,
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Select a district in the top bar to manage district expenditure.
          </p>
        </div>
        <SelectDistrictHint description="Choose a district from the top bar to view, import, and record expenditure." />
      </div>
    )
  }
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {activeAccounts.length === 0 && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-300">
          Add at least one active account in Accounts before recording expenditure. Imported rows will also require an account unless the district has exactly one active account.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 flex items-start gap-4">
          ...
          <div>
            <p className="text-sm text-slate-400">{selectedDistrictName} expenditure</p>
            {Object.entries(totalsByCurrency).length === 0 ? (
              <p className="text-2xl font-bold text-red-400 mt-0.5">-</p>
            ) : (
              Object.entries(totalsByCurrency).map(([currency, amount]) => (
                <p key={currency} className="text-xl font-bold text-red-400 mt-0.5">
            ...
            <table className="w-full text-sm">
                    ...
                    >
                      ...
                      <td className="px-4 py-2 text-slate-400 min-w-[190px]">
                        {isEditing ? (
                          ...
                          <span className={expense.account?.name ? 'text-slate-300' : 'text-slate-600'}>
                      ...
                      <td className="px-4 py-2 min-w-[110px]">
                        {isEditing ? (
                          ...
                          <span className="text-slate-400 capitalize">{PAYMENT_METHOD_LABELS[expense.payment_method]}</span>
                        )}
                      </td>
              ...
              <tfoot>
                <tr className="border-t border-slate-700 bg-slate-900/50">
                  ...
                  <td className="px-4 py-3 text-right font-bold text-red-400 space-y-0.5">
                    {Object.entries(totalsByCurrency).map(([currency, amount]) => (
