...
import { TrendingUp, TrendingDown, Scale, Download, FileDown, FileType2 } from 'lucide-react'
...
export default function ReportsPage() {
  ...
  const handleExport = () => {
    ...
    rows.push({ Section: 'Income & Expenditure Statement', Date: reportDate, District: districtName, Category: '', Description: '', Amount: '', Type: '' })
    rows.push({ Section: '', Date: '', District: '', Category: '', Description: '', Amount: '', Type: '' })
    rows.push({ Section: 'INCOME', Date: '', District: '', Category: '', Description: '', Amount: '', Type: '' })
    ...
    for (const [category, { items, amount }] of Object.entries(incomeByCategory).sort(([, a], [, b]) => b.amount - a.amount)) {
      ...
      rows.push({ Section: '', Date: '', District: '', Category: '', Description: `${category} subtotal`, Amount: amount, Type: 'subtotal' })
    }

    rows.push({ Section: '', Date: '', District: '', Category: '', Description: 'TOTAL INCOME', Amount: totalIncome, Type: 'total' })
    rows.push({ Section: '', Date: '', District: '', Category: '', Description: '', Amount: '', Type: '' })
    rows.push({ Section: 'EXPENDITURE', Date: '', District: '', Category: '', Description: '', Amount: '', Type: '' })
    ...
    rows.push({ Section: '', Date: '', District: '', Category: '', Description: 'TOTAL EXPENDITURE', Amount: totalExpenses, Type: 'total' })
    rows.push({
      Section: '',
      Date: '',
      District: '',
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {totalIncome === 0 && totalExpenses === 0 && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-12 text-center">
          <p className="text-slate-500">No financial data yet. Add income or expenditure to see reports.</p>
        </div>
      )}
    </div>
  )
